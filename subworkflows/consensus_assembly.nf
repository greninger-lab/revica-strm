...
workflow CONSENSUS_ASSEMBLY {                                                 
    ...
    emit:
    consensus   = IVAR_CONSENSUS_BBMAP_ALIGN_2.out.consensus // channel: [ val(meta), val(ref_info), path(consensus) ]
    bam         = IVAR_CONSENSUS_BBMAP_ALIGN_2.out.bam       // channel: [ val(meta), val(ref_info), path(bam), path(bai) ]
    reads       = IVAR_CONSENSUS_BBMAP_ALIGN_2.out.reads     // channel: [ val(meta), path(reads) ]
} 
